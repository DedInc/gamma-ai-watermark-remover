...
def key_to_pptx(key_path: str, pptx_path: str) -> dict:
    ...
    delay 1
    export theDoc to POSIX file {_escape_applescript_path(pptx_path)} as Microsoft PowerPoint
...
'''
    try:
        ...
        if not os.path.isfile(pptx_path):
            return {"success": False, "error": "Keynote did not produce a .pptx output file."}

        logger.info(f"key_to_pptx: '{key_path}' → '{pptx_path}'")
        return {"success": True, "error": None}

    except subprocess.TimeoutExpired:
        return {"success": False, "error": "Keynote conversion timed out (>120 s). Try a smaller file."}
    except Exception as e:
        return {"success": False, "error": f"Unexpected error during Keynote conversion: {e}"}
...
'''
    try:
        ...
        if not os.path.exists(key_path):
            return {"success": False, "error": "Keynote did not produce a .key output file."}
