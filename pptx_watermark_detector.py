...
class PPTXWatermarkDetector:
        ...
        try:
            ...
            for master_idx, master in enumerate(prs.slide_masters):
                ...
                for layout_idx, layout in enumerate(master.slide_layouts):
                    layout_name = layout.name if layout.name else f"Layout{layout_idx + 1}"
            ...
            if watermark_count > 0:
                ...
                for r in results:
                    if r['is_watermark']:
                        logger.info(f"  ✓ {r['location_name']}: {r['shape_name']} ({r['hyperlink']})")
            else:
    ...
    def _check_shapes(self, shapes, slide_width, slide_height, location_type, location_name):
        """
        ...
        for shape in shapes:
            ...
            is_in_corner = (left_pct >= self.corner_threshold and top_pct >= self.corner_threshold)
            ...
            if is_watermark:
                logger.info(f"    ✓ Found watermark: {shape.name} at ({left_pct*100:.1f}%, {top_pct*100:.1f}%) -> {hyperlink_url}")
            else:
                logger.debug(f"    Corner image without gamma link: {shape.name} at ({left_pct*100:.1f}%, {top_pct*100:.1f}%)")
