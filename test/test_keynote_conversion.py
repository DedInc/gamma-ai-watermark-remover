...
pytestmark = pytest.mark.skipif(
    sys.platform != "darwin", reason="Keynote conversion requires macOS and Keynote.app"
)
...
def test_key_to_pptx(tmp_path: Path) -> None:
...
def test_pptx_to_key(tmp_path: Path) -> None:
    ...
    assert r1["success"] is True, (
        f"key_to_pptx (prerequisite) failed: {r1.get('error')}"
    )
